...
def test_production_api():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with open(test_pdf, 'rb') as f:
            files = {'carrier': ('test.pdf', f, 'application/pdf')}
            data = {
                'content': 'PRODUCTION SAFETY TEST MESSAGE - This message verifies that the document corruption bug has been completely fixed.',
                'password': ''
            }
            
            response = requests.post(f"{API_BASE}/embed", files=files, data=data, timeout=30)
        ...
        if response.status_code == 200:
            ...
            if embedded_integrity:
                ...
                with open(embedded_pdf, 'rb') as f:
                    ...
                    response = requests.post(f"{API_BASE}/extract", files=files, data=data, timeout=30)
                
                if response.status_code == 200:
                    result = response.json()
                    if result.get('success'):
                        extracted_message = result.get('extracted_data', '')
                        expected_message = 'PRODUCTION SAFETY TEST MESSAGE - This message verifies that the document corruption bug has been completely fixed.'
                        ...
                        if extracted_message == expected_message:
                            ...
                            print(f"❌ Extraction mismatch: {extracted_message}")
                    else:
