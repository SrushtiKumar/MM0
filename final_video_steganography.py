...
class SafeVideoSteganography:
    ...
    def _detect_video_format(self, video_path: str) -> str:
    ...
    def hide_data_in_video(self, video_path: str, secret_data: Union[str, bytes], 
        ...
        # Detect video format
        video_format = self._detect_video_format(video_path)
        print(f"[SAFE VIDEO] Detected format: {video_format}")
        
        # Warning for problematic formats
        if video_format == "AVI":
            print(f"[SAFE VIDEO] ⚠️ AVI format detected - audio may not play properly after steganography")
            print(f"[SAFE VIDEO] 💡 Recommendation: Use MP4 format for best compatibility")
        ...
        return {
            ...
            'compatibility_warning': 'AVI format may have audio playback issues' if video_format == 'AVI' else None
        }
    ...
    def _encrypt_data(self, data: bytes) -> bytes:
        ...
        # Derive key - safely handle password type
        password_str = str(self.password) if not isinstance(self.password, str) else self.password
...
class FinalVideoSteganographyManager:
    ...
    def __init__(self, password: str = ""):
        self.safe_stego = SafeVideoSteganography(password)
    ...
    def hide_data(self, carrier_file_path: str, content_to_hide: Union[str, bytes], 
        ...
        # Set password if provided - ensure it's a string
        if password:
            self.safe_stego.password = str(password) if not isinstance(password, str) else password
    ...
    def extract_data(self, stego_file_path: str, password: Optional[str] = None, 
        ...
        if result and output_dir:
            ...
            return {
                'success': True,
                'extracted_data': data,  # Always return raw bytes - let API handle decoding
                'filename': filename,
